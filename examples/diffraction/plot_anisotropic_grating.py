...
for jangle, angle in enumerate([0,-20,-40]):
    ...
    for i in range(nper):
        ...
        f = E * phase_shift(i)
        cbTE = dolfin.plot(
            f.real, cmap="RdBu_r", transform=t + ax[jangle][0].transData, vmin=vminTE, vmax=vmaxTE
        )
    ...
    for i in range(nper):
        ...
        f = H * phase_shift(i)
        cbTM = dolfin.plot(
            f.real, cmap="RdBu_r", transform=t + ax[jangle][1].transData, vmin=vminTM, vmax=vmaxTM
        )
        plot_subdomains(grating.markers, domain=id, shift=(i * d, 0))
