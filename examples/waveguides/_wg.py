...
def run(case):
    ...
    for i, kt in enumerate(k0t):
        ...
        k_target = wavenumber * ncore * 1.02  # if i==0 else evs[0].real*1.1
        simu = gy.Waveguide(
            geom,
            epsilon=epsilon,
            wavenumber=wavenumber,
            # boundary_conditions=dict(outer_boundaries="PEC"),
            degree=(1, 1),
            # pml_stretch=1,
        )
...
#     for j in range(3):
#         plt.sca(ax[0][j])
#         mappa = gy.dolfin.plot(E[j].real, cmap="RdBu_r")
#         plot_geom(color="k")
#         plt.xlim(-box_width / 2, box_width / 2)
#         plt.ylim(-htot / 2, htot / 2)
#         plt.title(r"Re " + title[j])
#         plt.colorbar(mappa)
#         plt.axis("off")
#     for j in range(3):
#         plt.sca(ax[1][j])
#         mappa = gy.dolfin.plot(E[j].imag, cmap="RdBu_r")
#         plot_geom(color="k")
#         plt.xlim(-box_width / 2, box_width / 2)
#         plt.ylim(-htot / 2, htot / 2)
#         plt.title(r"Im " + title[j])
#         plt.colorbar(mappa)
#         plt.axis("off")
#     plt.suptitle(rf"$n_{{eff}}=${neff[i]:.3f}")
#     plt.tight_layout()
...
#     E = modes[i]
#     El = E[2]
#     Et = gy.vector([E[0], E[1]])
#     vec = Et.real
#     gy.dolfin.plot(El.real, cmap="RdBu_r")
#     gy.dolfin.plot(vec, cmap="Greys")
#     plot_geom("k")
#     plt.xlim(-box_width / 2, box_width / 2)
#     plt.ylim(-htot / 2, htot / 2)
#     plt.title(r"Re $E_z")
